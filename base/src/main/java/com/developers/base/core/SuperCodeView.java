 ...
 * @Description
 */
public abstract class SuperCodeView extends RelativeLayout implements Camera.PreviewCallback {
    ...
    public SuperCodeView(Context context) {
        super(context);
    }

    public SuperCodeView(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    public SuperCodeView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        mCameraPreview.setId(R.id.super_camera_preview);
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        if (SuperCodeUtil.isDebug()) {
            SuperCodeUtil.d("两次 onPreviewFrame 时间间隔：" + (System.currentTimeMillis() - mLastPreviewFrameTime));
        ...
        mProcessDataTask = new ProcessDataTask(camera, data, this, SuperCodeUtil.isPortrait(getContext())).perform();
    }
    ...
    private void handleAmbientBrightness(byte[] data, Camera camera) {
        ...
        if (Math.abs(data.length - pixelCount * 1.5f) < 0.00001f) {
            ...
            SuperCodeUtil.d("摄像头环境亮度为：" + cameraLight);
            if (mDelegate != null) {
    ...
    void onPostParseData(ScanResult scanResult) {
        ...
        if (TextUtils.isEmpty(result)) {
            try {
                if (mCamera != null) {
                    mCamera.setOneShotPreviewCallback(SuperCodeView.this);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    protected boolean transformToViewCoordinates(final PointF[] pointArr, final Rect scanBoxAreaRect, final boolean isNeedAutoZoom, final String result) {
        ...
        try {
            ...
            int statusBarHeight = SuperCodeUtil.getStatusBarHeight(getContext());
